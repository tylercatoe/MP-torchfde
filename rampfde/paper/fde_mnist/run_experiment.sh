#!/bin/bash
# Full 30-epoch experiment comparing torchfde FP32 vs rampde L1 FP16 vs
# rampde predictor FP16 on MNIST.
...
conda run -n implicit-oc python train_fde_mnist.py \
  ...
  --T "$T" --step_size "$STEP" --beta "$BETA" \
  --dim "$DIM" \
  --save "results/rampde_predictor_fp16_b${BETA}_T${T}"

echo ""
echo "=== All three runs complete. Compare with: ==="
echo "  python compare_results.py results/torchfde_fp32_b${BETA}_T${T}/results.json results/rampde_fp16_b${BETA}_T${T}/results.json results/rampde_predictor_fp16_b${BETA}_T${T}/results.json"
