...
submit_run() {
  ...
  local job_id
  job_id=$(sbatch --parsable --job-name="fashion-${label}" \
...
echo "Submitting the canonical 13-run FashionMNIST matrix (epochs=$epochs)..."
...
analysis_job=$(sbatch --parsable \
  --dependency="afterok:${dependency}" --job-name=fashion-analysis \
