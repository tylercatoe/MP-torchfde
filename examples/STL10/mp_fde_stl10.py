...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train/benchmark FDE-based STL10 model with multiple backprop modes",
    ...
    parser.add_argument(
        ...
        help="Direct method. 'auto' maps from adjoint base name.",
    )
    parser.add_argument(
        "--mp-dtype",
        type=str,
        ...
        help="Autocast dtype for adjoint-mixed mode",
    )
    parser.add_argument(
        "--mp-loss-scaler",
        type=str,
        default="auto",
        choices=["auto", "dynamic", "false"],
        help="Loss scaler strategy for adjoint-mixed mode",
    )
...
class MPFDE_STL10(nn.Module):
    ...
    def __init__(
        ...
        t_grid = torch.linspace(0.0, float(fde_config.T), int(time_bins) + 1)
        ...
        # 2) FDE block #1
        self.fde1 = FDEBlock(ODEFunc(ch, t_grid, is_stable=is_stable), fde_config, fdeint_solver)
        ...
        # 4) FDE block #2
        self.fde2 = FDEBlock(ODEFunc(2 * ch, t_grid, is_stable=is_stable), fde_config, fdeint_solver)
        ...
        # 5) FDE block #3
        self.fde3 = FDEBlock(ODEFunc(4 * ch, t_grid, is_stable=is_stable), fde_config, fdeint_solver)
...
def train_one_mode(
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        T_max=args.nepochs * batches_per_epoch,
        eta_min=1e-4,
    )
    ...
    if not np.isfinite(last_val_acc):
        last_val_acc = evaluate_accuracy(
            model,
            val_loader,
            device,
            autocast_dtype=mode_cfg.autocast_dtype,
    ...
    return {
        ...
        "inference_val_acc": float(inference_acc),
    }
